...
function toggleSolve(id, e) {
  e.stopPropagation();
  const wasSolved = solved.has(id);
  if (wasSolved) solved.delete(id); else solved.add(id);
  saveSolved();
  if (!wasSolved) recordSolveForStreak(); // only on marking solved
  updateStats();
  render();
}
...
const STREAK_KEY = 'dsahub_streak';
...
function loadStreak() {
  try {
    return JSON.parse(localStorage.getItem(STREAK_KEY)) || {
      current: 0, best: 0, lastSolveDate: null, solvedDates: []
    };
  } catch { return { current: 0, best: 0, lastSolveDate: null, solvedDates: [] }; }
}
...
function recordSolveForStreak() {
  ...
  if (!data.solvedDates.includes(today)) {
    ...
    data.solvedDates = data.solvedDates.slice(-30);
  }

  if (!data.lastSolveDate) {
    data.current = 1;
  } else {
    const diff = daysBetween(data.lastSolveDate, today);
    if (diff === 0) { /* already counted today */ }
    else if (diff === 1) { data.current += 1; }
    else { data.current = 1; }
  }
  ...
  saveStreak(data);
  renderStreakWidget(true);
}

function checkStreakReset() {
  const data = loadStreak();
  if (!data.lastSolveDate) return;
  if (daysBetween(data.lastSolveDate, getTodayStr()) > 1) {
    data.current = 0;
    saveStreak(data);
  }
}

function renderStreakWidget(pulse) {
  const data = loadStreak();
  const today = getTodayStr();

  const numEl = document.getElementById('streak-num');
  if (numEl) {
    numEl.textContent = data.current;
    if (pulse) {
  ...
  if (subEl) {
    if (data.current === 0) subEl.textContent = 'Start solving today!';
    else if (data.current === 1) subEl.textContent = 'day streak — keep going!';
    else subEl.textContent = 'days in a row \uD83C\uDFAF';
  }

  const bestEl = document.getElementById('streak-best');
  if (bestEl && data.best > 0) {
    bestEl.innerHTML = 'Best: <span>' + data.best + ' day' + (data.best !== 1 ? 's' : '') + '</span>';
  }

  const dotsEl = document.getElementById('streak-dots');
  const labelsEl = document.getElementById('streak-dot-labels');
  if (dotsEl && labelsEl) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const active = data.solvedDates.includes(ds);
      const isToday = ds === today;
      dots += '<div class="streak-dot' + (active ? ' active' : '') + (isToday ? ' today' : '') + '" title="' + ds + '">' + (active ? '\u2713' : '') + '</div>';
      labs += '<span>' + LABELS[d.getDay()] + '</span>';
    }
    dotsEl.innerHTML = dots;
    labelsEl.innerHTML = labs;
  }
}
